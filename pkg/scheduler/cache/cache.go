...
import (
	...
	nrtversioned "github.com/k8stopologyawareschedwg/noderesourcetopology-api/pkg/generated/clientset/versioned"
...
type SchedulerCacheParams struct {
	...
	NodeResourceTopologyClient  nrtversioned.Interface
...
func newSchedulerCache(schedulerCacheParams *SchedulerCacheParams) *SchedulerCache {
	...
	// The NRT informer is only created when the cluster serves the NRT CRD;
	// otherwise its list/watch would block cache sync forever.
	if schedulerCacheParams.NodeResourceTopologyClient != nil && featuregates.NodeResourceTopologyEnabled() {
		sc.nrtInformerFactory = nrtinformers.NewSharedInformerFactory(schedulerCacheParams.NodeResourceTopologyClient, 0)
	}
